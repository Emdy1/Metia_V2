...
class SyncService extends ChangeNotifier {
  ...
  Future<void> mergeServerData(Map<String, dynamic> data) async {
    ...
    for (var animeJson in animes) {
      ...
      await animeDatabaseService.addAnimeDatabases2(
        AnimeDatabase().fromJson(animeJson),
      ); //this is so the logic that searches for the matched anime gets saved so no more guessing correct matched anime
    }
    for (var epJson in episodes) {
      ...
      await episodeDataService.addEpisodeData(
        EpisodeData().fromJson(epJson),
      ); //this seaves the current progress of the user an in episdoe in seconds.
    }
    ...
    for (var histJson in history) {
      ...
      await episodeHistoryService.addEpisodeHistory(
        EpisodeHistoryInstance().fromJson(histJson),
      ); // this is to show the history of the user's watched anime
    }
